...
- 可将 RhyLive ARKit 原始 UDP 数据包完整本地转发到 `127.0.0.1:17891`
...
- 设置接收地址、端口、面部来源和应用频率
- 开启 `本地转发 RhyLive 数据` 后会立即独立监听 RhyLive 输入端口，将原始 UDP 包转发到本机 `17891` 端口；不需要启动插件接收器
- `停止接收` 只停止角色预览与驱动；本地转发会持续运行，直到关闭转发开关或卸载扩展
- 点击 `开始接收`
- 如需临时停流，使用 `暂停接收`
...
3. 如需重录，先执行 `清空范围内关键帧`
4. 点击 `开始录制`
5. 保持 `快速保存` 开启，以更快完成大量关键帧写入；低响应压力场景可关闭
